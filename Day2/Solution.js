...
const data = perLine(input);

//Challenge 1 Solution
function puzzle1(arr) {
    let count = 0
    arr.forEach((string) => {
        let eachInput = string.split(" "),
            limit = eachInput[0].split("-"),
            letter = eachInput[1].replace(/:/, ""),
            word = [...eachInput[2]].filter(char => char === letter)

        if (!(word.length < limit[0]) && !(word.length > limit[1])) {
            count++
        }
    })
    return count
...
function puzzle2(arr) {
    let count = 0
    arr.forEach((string) => {
        let eachInput = string.split(" "),
            position = eachInput[0].split("-"),
            letter = eachInput[1].replace(/:/, ""),
            word = [...eachInput[2]]

        if (!(word[Number(position[0]) - 1] === letter && word[Number(position[1]) - 1] === letter)) {
            if (word[Number(position[0]) - 1] === letter || word[Number(position[1]) - 1] === letter) {
                count++
            }
        }
